...
class GA:
    ...
    @classmethod
    # Evolve pop
    # O(n^2): The code snippet contains two loops that iterate over the population size, resulting in a quadratic time complexity.
    ...
    # O(n^2) The code contains nested loops that iterate over the number of trucks and the route lengths
    @classmethod
    ...
    # Mutation opeeration
    # O(n) The code snippet contains while loops that iterate until certain conditions are met, and the number of iterations is dependent on the input size. Therefore, the time complexity is O(n), where n is the input size.
    @classmethod
